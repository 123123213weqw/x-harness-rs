...
- [ ] 退避策略：指数退避、抖动、Retry-After、每步总恢复期限；等待可取消且可在 UI 观察。
- [ ] 部分输出后的受控恢复：保留片段，明确新尝试边界与计费，不盲目拼接或重放工具。
- [ ] 轻量网络诊断：最近网络块时间、协议结束状态、路由信息；不要求常开完整内容 Trace。
- [ ] 真实网络切换/TLS 中断验收；发布并安装包含本轮网络修复的桌面包。
