...
当前资源保护上限：单图 20 MiB、16 MiPixels、解码分配 128 MiB；模型单次请求中图片原始字节合计 40 MiB。没有会话累计张数限制，也不再以固定 16 张拒绝请求。
组装每次模型可见上下文时优先保留最新图片；若重放的历史图片超过 40 MiB，较早的图片只在本次请求中替换为明确的 `attachment://<id>` 引用。原图与完整历史仍留在会话和 UI 中；模型需要重新查看时使用既有 `read(path="attachment://<id>")`，由会话归属校验后重新加入图片。此投影发生在 token 计量之前，保证计量和发送的模型输入一致。Provider 仍对单次请求执行 40 MiB 兜底检查；它不是对话生命周期上限。
