...
日常入口见[统一发布任务](release-orchestrator.md)：复用本规范的工作流与全部门禁，正式发布需单独确认版本。

> 2026-09-08。本规范区分“代码可用”“CI 验收通过”“正式更新已发布”。
> 三者不能互相替代。验收结束时 Windows 原通道为 friends-v0.2.6；Mac/Linux 的正式
> 发布必须满足所选策略的门禁。显式 `all` 缺少 Apple 凭据时明确阻断；
> 只有显式 `all-macos-preview` 允许未公证 Mac 内测包，不会自动降级。

## 2026-09-15：默认 Mac 预览发布，保留显式 Windows/Linux 范围
...
macOS 固定测试通道。显式 `all` 仍要求四个平台及 Apple 正式凭据，不会自动降级；
本仓库已明确选择未公证内测，标签触发和手动默认按 `all-macos-preview` 解析（见「操作」）。
这是显式发布策略，不是 Apple 签名失败后的自动降级。
...
发布将被 Promote 拒绝，必须恢复 `all` / `all-macos-preview` 或先设计独立平台通道，不能静默删除 Mac。
旧无 release_scope 的计划按 all 验证，旧客户端协议、版本号格式、公钥和标识不变。

操作：创建已通过精确 master CI 的 desktop-vX.Y.Z 标签。范围按 `inputs.release_scope`
→ 仓库变量 `XHARNESS_DESKTOP_TAG_RELEASE_SCOPE` → `all-macos-preview` 依次解析：标签推送
没有输入，因此缺 Apple 凭据时不再产生必然失败的 all 运行，标签本身即可构建出候选；
也可继续从 master 显式 dispatch Desktop Release（release_tag=该标签，
release_scope=windows-linux），两者产物一致。显式 `all` 在缺 Apple 凭据时仍在构建前
拒绝，不能作为候选或发布证据。记录成功的 scoped 构建 Run ID，继续原来的
Windows + Unix(candidate) 验收和 Promote，不能跳过任一项。
首次 Linux 用户仍需安装新的 AppImage 基础包；macOS 用户保持原样。

以下“四个平台”描述是显式 all 范围的要求；windows-linux 对应上述严格两平台集合。
...
- 显式 `all` 的 Developer ID/Gatekeeper/公证断言仍完整保留。内测验收要求
...
标签触发按 `windows-linux` 解析，既不会选 `all`，也不会因 Apple 凭据缺失自动选择内测。仍必须使用新版本号，
...
通过 GitHub Actions Secrets 配置，不能在对话中粘贴私钥。显式 `all` 证书和公证失败时
...
显式 `all` 无 Apple 凭据时第 2 步应阻断，这是设计行为。标签触发与手动运行默认
取 `all-macos-preview`；须显式选择 `all` 或 `windows-linux` 才使用其他策略。无 Secret 的 CI `rehearsal` 构件只能作
测试证据，不提供给用户当作正式基础包，也不能拿其 Run ID 通过正式发布门禁。

验收控制工作流从可信 master 启动，再显式 checkout 候选构建 SHA；发布期间主干可继续
前进，但构建、被测程序与验收源码绑定仍保持一致。普通发布不修改旧版本固定测试地址；旧用户可使用专门的受控桥接迁移，
见 [Mac 预览更新通道](macos-preview-updates.md)，无需把手动覆盖视为唯一迁移方法。
...
## 2026-09-10：原生验收退出阶段的混合 UID 竞态

主分支 CI 与 0.2.15 Linux 候选验收在完成安装、重启、数据校验后，向已验证同 UID
进程组发送 TERM，随即观察到短暂 UID 0 子进程。它符合 AppImage/FUSE 卸载辅助程序的
退出时序，但不能仅凭 UID/进程名认定其归属。

验收器只在此前已经对同 UID 组发过退出信号时，允许最多 5 秒**纯观察等待**。
一旦观察到混合 UID，不再发送 TERM/KILL，即使后续快照恢复同 UID 也不再发送。
只有整个组没有活进程、直接子进程已回收才算成功。起始快照就出现外 UID、等待超时、
进程清单不可读等仍失败。数字身份快照及等待标记写入 cleanup.json，不记录命令或环境。
这不修改产品权限、Updater 签名或验收断言；正式候选仍须连续三轮原生升级验收。

### 验收驱动与浏览器自动检查隔离

原生验收的第三轮曾在签名损坏测试后失败：UI 的启动后 1.5 秒自动检查与原生驱动的
下载操作争抢同一更新锁，驱动将正常的 busy 拒绝 unwrap 成 panic。准备阶段仅在
**一次性基础包副本**移除 UI 启动/周期自动检查定时器；事件订阅和所有生产 Handler
保持不变，原生驱动仍明确测试两个并发请求只有一个成功。正式目标包的 UI、自动检查、
签名和字节摘要都不改。缺失或漂移的注入锚点直接失败，不能静默跳过隔离。
