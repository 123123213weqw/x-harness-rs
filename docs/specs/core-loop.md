...
- Provider Delta 先实时进入进程内 Loop Event；写入 Durable Session 前可以合并同 Turn/Step、
  同类型的相邻 Text/Reasoning 碎片。持久检查点由 64 个原始碎片、4 KiB 合并内容或 250 ms
  任一阈值触发；原始碎片计数不能因合并而失去崩溃恢复上界。
- 首次请求失败后默认最多重试两次。
