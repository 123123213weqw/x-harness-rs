...
持续有数据的长推理不会因为超过 600 秒直接被终止。重试已加入指数退避（500ms 基数、
8s 本地上限、上下 20% 抖动），Retry-After 秒数/HTTP 日期是不可提前的等待下界。
第一次可重试故障开始计算 60s 恢复期限，覆盖后续等待、连接和未产生 Delta 的流。
初次请求仍使用 Provider 超时；首次 Delta 后恢复期限不再限制正常长生成。
暂停期间不发请求，暂停时间计入期限，在用户恢复时重新检查。
...
新增隔离 TLS 服务器实际省略 `close_notify`，覆盖输出前、部分输出、协议已完成三个边界。
证书校验保持开启，仅测试 Client 信任临时证书。没有切换用户物理 Wi-Fi，不能替代所有
运营商/代理路径验收。详见 `docs/specs/network-recovery.md`。
