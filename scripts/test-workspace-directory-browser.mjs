...
try {
  ...
  await page.evaluate(() => {
    ...
    window.base = 'D:\\工作区'
    window.paths = new Map([[base, ['已有项目', '.hidden']], [base + '\\已有项目', []], [base + '\\.hidden', []], ['C:\\', ['Users']], ['\\\\server\\share', []], ['/tmp/projects', []]])
    ...
    const listing = path => ({ path, home: base, entries: (paths.get(path) ?? []).map(name => ({ name, path: path + '\\' + name, hidden: name.startsWith('.') })),
  ...
  assert.equal(await path().evaluate(element => element === document.activeElement), true, 'focus must stay in picker')
  ...
  console.log(`${engine}: shipped sidebar/hero, real Modal/Button, existing/new workspace, errors/retry, paths, late reads/cancel, duplicate create guard, IME and layout passed`)
