...
for (const entry of composed) {
  ...
  if (entry.name === '@deepseek-ai/dsh-client-ui-conversation') {
    ...
    bytes = patchConversationMessageEdit(bytes)
  }
...
for (const entry of entries) {
  ...
  try {
    if (!['@deepseek-ai/dsh-client-ui-conversation', '@deepseek-ai/dsh-client-ui-model-selection', '@deepseek-ai/dsh-client-connection'].includes(entry.id)) writeFileSync(`${target}.map`, portableBytes(readFileSync(sourceMap)))
  } catch (error) {
