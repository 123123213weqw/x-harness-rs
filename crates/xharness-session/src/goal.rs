...
pub use crate::{GoalBlockReason, GoalPhase, GoalSnapshot, Revision};
...
pub struct GoalExecutionState {
    ...
    pub activation_epoch: u64,
    pub rounds_started: u64,
    pub pending: Option<GoalIntent>,
    pub running: Option<GoalRoundClaim>,
    pub latest_turn: Option<GoalTurnResult>,
    pub admitted: BTreeSet<ContinuationKey>,
    pub empty_report_rounds: u32,
    pub empty_report_limit: u32,
...
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct GoalRoundClaim {
    pub intent: GoalIntent,
    pub turn: u32,
}
#[derive(Clone, Copy, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum GoalExecutionOperation {
    Enable,
    Enqueue,
    Claim,
    Settle,
...
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct GoalExecutionChange {
    pub version: u8,
    pub operation: GoalExecutionOperation,
    pub state: GoalExecutionState,
}

/// Rebuild only from the session log. A legacy control mutation invalidates
/// execution authorization, but retains round accounting and unsettled claims.
pub fn execution_state(session: &crate::Session) -> Option<GoalExecutionState> {
    let mut state: Option<GoalExecutionState> = None;
    for e in session.events() {
        match e.data() {
            crate::EventData::GoalExecution { change } => state = Some(change.state.clone()),
            crate::EventData::GoalChange {
                change: crate::GoalChange::Snapshot(c),
            } => {
                if let Some(s) = state.as_mut() {
                    if s.definition.snapshot.id != c.goal.id {
                        state = None;
                    } else {
                        if c.version == 1 {
                            s.definition.execution_enabled = false;
                        }
                        s.definition.snapshot = c.goal.clone();
                        s.rounds_started = c.rounds_started;
                    }
                }
            }
            crate::EventData::GoalChange {
                change: crate::GoalChange::Clear(_),
            } => state = None,
            _ => {}
        }
    }
