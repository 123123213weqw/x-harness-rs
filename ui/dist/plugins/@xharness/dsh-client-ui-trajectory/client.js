window.__ModuleLoader__.load({
	id: "@xharness/dsh-client-ui-trajectory",
	factory: (require) => {
		var module = { exports: {} };
		...
		*/
		function createTrajectoryDurationStore() {
			return (0, _xharness_dsh_client_runtime_client.createSnapshotStore)(false, { persist: { name: "dsh.trajectory.duration" } });
		}
		//#endregion
		...
		*/
		...
		function updateChunk(state, match) {
			...
			switch (chunk.type) {
				case "block-start":
					blocks[chunk.index] = (0, _xharness_dsh_client_runtime_client.emptyAssistantBlock)(chunk.blockType);
					break;
				case "text-delta": {
				...
				case "block-end":
					blocks[chunk.index] = (0, _xharness_dsh_client_runtime_client.toAssistantBlock)(chunk.block);
					break;
				default: return {
					...state,
					sawChunk: true
			...
			return {
				...
				...(0, _xharness_dsh_client_runtime_client.isTokenDelta)(chunk) && state.firstTokenTime === void 0 ? { firstTokenTime: match.event.time } : {}
			};
		}
		...
		function fallbackState$1(context) {
			...
			for (const match of context.matches) {
				...
				if (event.type === "assistant/chunk") {
					...
					state = {
						...state,
						blocks: (0, _xharness_dsh_client_runtime_client.toAssistantBlocks)(event.data.message.content),
						final: match,
		...
		const trajectoryAssistantDefinition = {
			...
			update: (context, match) => {
				...
				if (match.event.type === "assistant/message") return {
					...context.state,
					blocks: (0, _xharness_dsh_client_runtime_client.toAssistantBlocks)(match.event.data.message.content),
					final: match,
				...
				return {
					...
					retry: {
						message: (0, _xharness_dsh_client_runtime_client.displayFailureMessage)(data.failure),
						retry: data.retry,
		...
		const trajectoryTurnEndDefinition = {
			...
			start: (_context, match) => {
				...
				return {
					...
					...reason.kind === "error" ? { error: (0, _xharness_dsh_client_runtime_client.displayFailureMessage)(reason.error) } : {}
				};
			},
		...
		const trajectoryMessageDefinition = {
			...
			start: (_context, match, reader) => {
				...
				if (event.data.source.kind !== "user") return {
					...
					provenance: (0, _xharness_dsh_client_runtime_client.contextProvenance)(event.data.source),
					form: (0, _xharness_dsh_client_runtime_client.contextForm)(event.data.source)
				};
 ...
 /* istanbul ignore else */
		...
		*/
		function trajectoryPreviewText(text) {
			...
			const compact = (0, _xharness_dsh_client_ui_primitives.extractMarkdownPlainText)(source).replace(/\s+/g, " ").trim();
		...
		//#endregion
		//#region \0dsh-css:deepseek-harness/packages/client/ui-trajectory/src/client/TrajectoryTable.module.css.mjs
		const css$3 = "._8DyahG_split{--dsh-scrollbar-thumb:var(--dsw-alias-scrollbar-bg-l2);--dsh-scrollbar-thumb-hover:var(--dsw-alias-scrollbar-hover-l2);background:var(--dsw-alias-bg-layer-1);flex:1;width:100%;min-height:0;display:flex;position:relative;overflow:hidden;container-type:inline-size}._8DyahG_tablePane{min-width:0;padding-bottom:var(--dsh-trajectory-bottom-clearance,0px);flex:1;position:relative;overflow:hidden auto;container:_8DyahG_trajectory-table/inline-size}._8DyahG_historyLoading{z-index:5;pointer-events:none;height:0;position:sticky;top:0;overflow:visible}._8DyahG_historyLoadingBar{box-sizing:border-box;border-bottom:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1);width:100%;height:30px;color:var(--dsw-alias-label-secondary);font:var(--dsw-font-xxs-12);justify-content:center;align-items:center;gap:6px;display:flex}._8DyahG_historyLoadingSpinner{box-sizing:border-box;border:1.5px solid var(--dsw-alias-border-l2);border-top-color:var(--dsw-alias-state-business-primary);border-radius:50%;width:10px;height:10px;animation:.7s linear infinite _8DyahG_history-loading-spin}._8DyahG_table tbody ._8DyahG_historyLoadRow td{height:30px;padding:0}._8DyahG_table tbody ._8DyahG_historyLoadRow+tr[data-turn-start=true] td:before{content:none}._8DyahG_historyLoadButton{background:var(--dsw-alias-bg-layer-1);width:100%;height:29px;color:var(--dsw-alias-label-secondary);cursor:pointer;font:var(--dsw-font-xxs-12);border:0;justify-content:center;align-items:center;gap:6px;display:flex}._8DyahG_historyLoadButton:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}._8DyahG_historyLoadButton:focus-visible{outline:2px solid var(--dsw-alias-state-business-primary);outline-offset:-2px}._8DyahG_historyLoadButton:disabled{cursor:default}._8DyahG_visuallyHidden{clip:rect(0 0 0 0);white-space:nowrap;width:1px;height:1px;position:absolute;overflow:hidden}._8DyahG_table:not([data-scroll-ready=true]){visibility:hidden}@keyframes _8DyahG_history-loading-spin{to{transform:rotate(360deg)}}@media (prefers-reduced-motion:reduce){._8DyahG_historyLoadingSpinner{animation:none}}._8DyahG_table{--trajectory-turn-accent:color-mix(in srgb, var(--dsw-static-blue-500) 22%, var(--dsw-alias-bg-layer-1));border-spacing:0;table-layout:fixed;width:100%;min-width:0;color:var(--dsw-alias-label-primary);background:var(--dsw-alias-bg-layer-1);font:var(--dsw-font-xxs-12)}._8DyahG_eventColumn{width:122px}._8DyahG_contentColumn{width:auto}._8DyahG_table th{z-index:3;box-sizing:border-box;border-bottom:1px solid var(--dsw-alias-border-l2);height:30px;color:var(--dsw-alias-label-tertiary);background:var(--dsw-specific-sidebar-fill);font:var(--dsw-font-xxs-12);text-align:left;text-overflow:ellipsis;user-select:none;white-space:nowrap;padding:0 8px;font-weight:500;position:sticky;top:0;overflow:hidden}._8DyahG_eventHeader{text-align:right!important;padding-right:4px!important}._8DyahG_table td{box-sizing:border-box;border-bottom:1px solid var(--dsw-alias-border-l1);text-overflow:ellipsis;white-space:nowrap;height:30px;padding:0 8px;overflow:hidden}._8DyahG_table tbody ._8DyahG_virtualSpacer{pointer-events:none}._8DyahG_table tbody ._8DyahG_virtualSpacer td{height:var(--trajectory-virtual-spacer-height);border:0;padding:0}._8DyahG_table tbody tr:not([data-collapsed-summary]):not([data-virtual-spacer]):not([data-history-load]){cursor:default;transition:background-color .12s var(--ds-ease-in-out), opacity .12s var(--ds-ease-in-out);outline:none}._8DyahG_table tbody tr[data-timeline-focus=outside]{opacity:.24}._8DyahG_table tbody tr:not([data-collapsed-summary]):not([data-virtual-spacer]):not([data-history-load]):not([data-selected=true]):hover{background:var(--dsw-alias-interactive-bg-hover)}._8DyahG_table tbody tr[data-request-only=true]:hover{background:0 0}._8DyahG_table tbody tr[data-request-only=true] td{border-bottom:0;height:0;padding-top:0;padding-bottom:0}._8DyahG_table tbody tr[data-terminal-request-boundary=true] td{height:9px}._8DyahG_table tbody tr[data-request-only=true] ._8DyahG_turnRail{top:-15px;bottom:0}._8DyahG_table tbody tr:not([data-collapsed-summary]):focus-visible{box-shadow:inset 0 0 0 1px var(--dsw-alias-state-business-primary)}._8DyahG_table tbody tr[data-selected=true]{background:var(--dsw-alias-interactive-bg-active)}._8DyahG_requestBoundaryControl{--request-boundary-base-left:12px;z-index:6;top:-8px;left:calc(var(--request-boundary-base-left) + var(--request-boundary-offset,0px));cursor:pointer;background:0 0;border:0;width:16px;height:16px;padding:0;position:absolute}._8DyahG_requestBoundaryControl:before{background:var(--dsw-alias-label-caption);width:5px;height:5px;box-shadow:0 0 0 2px var(--dsw-alias-bg-layer-1), 0 0 0 3px transparent;content:\"\";transition:background .12s var(--ds-ease-in-out), box-shadow .12s var(--ds-ease-in-out);border-radius:50%;position:absolute;top:5.5px;left:5.5px}._8DyahG_requestBoundaryControl:after{border:1px solid var(--dsw-alias-border-l1);width:max-content;color:var(--dsw-alias-label-secondary);background:var(--dsw-alias-bg-layer-1);content:attr(data-label);font:9px/12px var(--ds-font-family-code);opacity:0;pointer-events:none;transition:opacity .12s var(--ds-ease-in-out), transform .12s var(--ds-ease-in-out);user-select:none;white-space:nowrap;border-radius:2px;padding:0 4px;position:absolute;top:2px;left:17px;transform:translate(-2px);box-shadow:0 2px 6px #0000001f}._8DyahG_requestBoundaryControl:hover:before,._8DyahG_requestBoundaryControl:focus-visible:before{background:var(--dsw-alias-brand-primary-new-colorprimary-new-color)}._8DyahG_requestBoundaryControlActive:before,._8DyahG_requestBoundaryControlActive:hover:before,._8DyahG_requestBoundaryControlActive:focus-visible:before{background:color-mix(in srgb, var(--dsw-alias-brand-primary-new-colorprimary-new-color) 18%, var(--dsw-alias-bg-layer-1));box-shadow:0 0 0 1.5px var(--dsw-alias-brand-primary-new-colorprimary-new-color)}._8DyahG_requestBoundaryControl[data-request-status=error]:before,._8DyahG_requestBoundaryControl[data-request-status=error]:hover:before,._8DyahG_requestBoundaryControl[data-request-status=error]:focus-visible:before{background:var(--dsw-alias-state-error-primary)}._8DyahG_requestBoundaryControl:hover:after,._8DyahG_requestBoundaryControl:focus-visible:after{opacity:1;transform:translate(0)}._8DyahG_requestBoundaryControl:focus-visible{outline:none}._8DyahG_table tbody tr:has(._8DyahG_requestBoundaryControl:hover):not([data-selected=true]){background:0 0}._8DyahG_event{position:relative}._8DyahG_turnRail,._8DyahG_selectionRail{background:var(--dsw-alias-brand-primary-new-colorprimary-new-color);pointer-events:none;position:absolute;left:0}._8DyahG_turnRail{z-index:4;background:var(--trajectory-turn-accent);width:2px;top:-1px;bottom:-1px}._8DyahG_table tbody tr[data-turn-end=true] ._8DyahG_turnRail{bottom:0}._8DyahG_selectionRail{z-index:5;width:3px;top:0;bottom:0}._8DyahG_table tbody tr[data-error=true] ._8DyahG_turnRail{background:color-mix(in srgb, var(--dsw-alias-state-error-primary) 22%, var(--dsw-alias-bg-layer-1))}._8DyahG_table tbody tr[data-error=true] ._8DyahG_selectionRail{background:var(--dsw-alias-state-error-primary)}._8DyahG_table tbody tr[data-turn-start=true] td{position:relative;overflow:visible}._8DyahG_table tbody tr[data-turn-start=true]:not(:first-child) td:before{z-index:1;background:var(--dsw-alias-border-l1);content:\"\";pointer-events:none;height:2px;position:absolute;top:0;left:0;right:0;transform:translateY(-50%)}._8DyahG_event{padding-left:36px!important;padding-right:4px!important;overflow:visible!important}._8DyahG_turnLabel{z-index:3;box-sizing:border-box;width:max-content;color:var(--dsw-alias-label-tertiary);background:var(--dsw-alias-bg-module-platform);font:8px/10px var(--ds-font-family-code);font-variant-numeric:tabular-nums;user-select:none;white-space:nowrap;border-radius:0 0 2px;flex:none;align-items:center;padding:1px 5px;display:inline-grid;position:absolute;top:0;left:0}._8DyahG_turnLabelFull,._8DyahG_turnLabelCompact{opacity:1;white-space:nowrap;grid-area:1/1;max-width:64px;overflow:hidden}._8DyahG_turnLabelCompact{opacity:0;max-width:0}._8DyahG_turnLabelActive{color:color-mix(in srgb, var(--dsw-static-blue-500) 55%, var(--dsw-alias-label-tertiary));background:var(--trajectory-turn-accent)}._8DyahG_eventInner{justify-content:flex-start;align-items:center;min-width:0;height:100%;display:flex}._8DyahG_kindSlot{flex:none;justify-content:flex-end;align-items:flex-end;width:76px;display:flex}._8DyahG_kindSlot [role=tooltip]{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);box-shadow:var(--dsw-shadow-lv2);color:var(--dsw-alias-label-primary)}._8DyahG_content{color:var(--dsw-alias-label-primary);padding-left:4px!important}._8DyahG_kindTag{box-sizing:border-box;letter-spacing:.035em;user-select:none;border:1px solid #0000;border-radius:4px;flex:none;align-items:center;height:19px;padding:0 5px;font-size:10px;font-weight:650;line-height:16px;display:inline-flex}._8DyahG_kindTagIcon{opacity:0;flex:none;justify-content:center;align-items:center;width:0;height:13px;display:inline-flex;overflow:hidden;transform:scale(.8)}._8DyahG_kindTagLabel{opacity:1;white-space:nowrap;max-width:72px;display:inline-block;overflow:hidden}._8DyahG_table ._8DyahG_kindSlot ._8DyahG_message{justify-content:center;width:100%}@container _8DyahG_trajectory-table (width<=620px){._8DyahG_eventColumn{width:50px}._8DyahG_event{padding-left:28px!important;padding-right:3px!important}._8DyahG_requestBoundaryControl{--request-boundary-base-left:6px}._8DyahG_kindSlot{width:19px}._8DyahG_kindTag,._8DyahG_table ._8DyahG_kindSlot ._8DyahG_message{justify-content:center;width:19px;padding-left:0;padding-right:0}._8DyahG_kindTagIcon{opacity:1;width:13px;transform:scale(1)}._8DyahG_kindTagLabel,._8DyahG_turnLabelFull{opacity:0;max-width:0}._8DyahG_turnLabelCompact{opacity:1;max-width:64px}}@media (prefers-reduced-motion:no-preference){._8DyahG_eventColumn,._8DyahG_event,._8DyahG_requestBoundaryControl,._8DyahG_kindSlot,._8DyahG_kindTag,._8DyahG_kindTagIcon,._8DyahG_kindTagLabel,._8DyahG_turnLabelFull,._8DyahG_turnLabelCompact{transition-duration:.18s;transition-timing-function:var(--ds-ease-in-out)}._8DyahG_eventColumn,._8DyahG_kindSlot{transition-property:width}._8DyahG_event{transition-property:padding-right,padding-left}._8DyahG_requestBoundaryControl{transition-property:left}._8DyahG_kindTag{transition-property:padding-right,padding-left}._8DyahG_kindTagIcon{transition-property:width,opacity,transform}._8DyahG_kindTagLabel,._8DyahG_turnLabelFull,._8DyahG_turnLabelCompact{transition-property:max-width,opacity}}._8DyahG_user{color:var(--dsw-alias-state-business-primary);background:var(--dsw-alias-state-business-tertiary)}._8DyahG_systemNeutral{color:var(--dsw-alias-label-secondary);background:var(--dsw-alias-bg-module-platform)}._8DyahG_contextGreen{color:color-mix(in srgb, var(--dsw-alias-state-success-primary) 68%, var(--dsw-alias-label-secondary));background:var(--dsw-alias-state-success-tertiary)}._8DyahG_compacted{color:var(--dsw-alias-label-secondary);background:var(--dsw-alias-bg-module-platform)}._8DyahG_compactedSummary{font:var(--dsw-font-xs-13);margin-top:12px;padding:0 0 14px}._8DyahG_promptDiffSections{flex-direction:column;gap:14px;max-height:100%;padding:10px 14px 14px;display:flex;overflow:auto}._8DyahG_promptDiffSection{min-width:0}._8DyahG_promptDiffTitle{color:var(--dsw-alias-label-secondary);font:var(--dsw-font-xs-strong-13);user-select:none;margin:0 0 6px}._8DyahG_promptDiff{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-bg-layer-1);font:11px/17px var(--ds-font-family-code);white-space:pre;margin:0;overflow:auto}._8DyahG_promptDiff span{min-width:max-content;padding:0 6px;display:block}._8DyahG_promptDiffLinemeta{color:var(--dsw-alias-label-caption);background:var(--dsw-alias-bg-module-platform);user-select:none}._8DyahG_promptDiffLinecontext{color:var(--dsw-alias-label-secondary)}._8DyahG_promptDiffLineadded{color:color-mix(in srgb, var(--dsw-alias-state-success-primary) 72%, var(--dsw-alias-label-primary));background:var(--dsw-alias-state-success-tertiary)}._8DyahG_promptDiffLineremoved{color:var(--dsw-alias-state-error-primary);background:color-mix(in srgb, var(--dsw-alias-state-error-primary) 12%, var(--dsw-alias-bg-layer-1))}._8DyahG_assistantVioletBright{color:color-mix(in srgb, var(--dsw-alias-brand-primary-new-colorprimary-new-color) 60%, var(--dsw-alias-state-error-secondary));background:color-mix(in srgb, color-mix(in srgb, var(--dsw-alias-brand-primary-new-colorprimary-new-color) 55%, var(--dsw-alias-state-error-secondary)) 15%, var(--dsw-alias-bg-layer-1))}._8DyahG_toolAmber{color:var(--dsw-alias-state-warn-label);background:var(--dsw-alias-state-warn-tertiary)}._8DyahG_subtoolAmber{color:color-mix(in srgb, var(--dsw-alias-state-warn-label) 62%, var(--dsw-alias-label-tertiary));background:color-mix(in srgb, var(--dsw-alias-state-warn-tertiary) 58%, var(--dsw-alias-bg-layer-1))}._8DyahG_contentText{text-overflow:ellipsis;white-space:nowrap;min-width:0;display:block;overflow:hidden}._8DyahG_toolCallOnly{color:var(--dsw-alias-label-tertiary)}._8DyahG_table tbody tr[data-collapsed-summary=turn] td,._8DyahG_table tbody tr[data-collapsed-summary=assistant] td{height:20px}._8DyahG_table tbody tr[data-collapsed-summary]{cursor:pointer;transition:background-color .12s var(--ds-ease-in-out);outline:none}._8DyahG_table tbody tr[data-collapsed-summary]:hover{background:var(--dsw-alias-interactive-bg-hover)}._8DyahG_table tbody tr[data-collapsed-summary]:focus-visible{box-shadow:inset 0 0 0 1px var(--dsw-alias-state-business-primary)}._8DyahG_collapsedTurnContent{min-width:0;color:var(--dsw-alias-label-secondary);align-items:center;font-size:12px;line-height:16px;display:flex}._8DyahG_collapsedTurnEllipsis{color:var(--dsw-alias-label-tertiary);user-select:none;flex:none;margin-right:6px;font-weight:600}._8DyahG_collapsedTurnText{text-overflow:ellipsis;white-space:nowrap;min-width:0;overflow:hidden}._8DyahG_resultPreview{grid-template-columns:clamp(180px, var(--trajectory-tool-request-width,calc(36cqw - 56px)), 480px) minmax(0, 1fr);align-items:center;gap:8px;min-width:0;display:grid}._8DyahG_resultRequest,._8DyahG_inlineResultText{text-overflow:ellipsis;white-space:nowrap;min-width:0;overflow:hidden}._8DyahG_toolCallNameTypeface{color:var(--dsw-alias-label-primary);font:400 12px/18px Menlo,Consolas,Liberation Mono,PingFang SC,Microsoft YaHei}._8DyahG_toolCallPayload{color:var(--dsw-alias-label-secondary);font:400 12px/18px var(--ds-font-family-code);margin-left:7px}._8DyahG_table tbody tr[data-kind=tool] ._8DyahG_contentText,._8DyahG_table tbody tr[data-kind=subtool] ._8DyahG_contentText,._8DyahG_table tbody tr[data-kind=tool] ._8DyahG_resultPreview,._8DyahG_table tbody tr[data-kind=subtool] ._8DyahG_resultPreview{font-family:var(--ds-font-family-code);font-size:12px}._8DyahG_table tbody tr[data-kind=subtool] ._8DyahG_content{padding-left:26px}._8DyahG_inlineResult{min-width:0;color:var(--dsw-alias-label-secondary);align-items:center;display:flex}._8DyahG_noOutputText{color:var(--dsw-alias-label-caption)}._8DyahG_arrow{color:var(--dsw-alias-label-caption);flex:none;margin-right:8px}._8DyahG_error,._8DyahG_overview dd._8DyahG_error,._8DyahG_details ._8DyahG_errorPayload{color:var(--dsw-alias-state-error-primary)}._8DyahG_details ._8DyahG_errorPayload ._8DyahG_resultBlockText{color:inherit}._8DyahG_details ._8DyahG_jsonPayload._8DyahG_errorPayload,._8DyahG_details ._8DyahG_jsonPreview._8DyahG_errorPayload{--json-tree-property:var(--dsw-alias-state-error-primary);--json-tree-string:var(--dsw-alias-state-error-primary);--json-tree-number:var(--dsw-alias-state-error-primary);--json-tree-keyword:var(--dsw-alias-state-error-primary);--json-tree-punctuation:var(--dsw-alias-state-error-primary);--json-tree-icon:var(--dsw-alias-state-error-primary)}._8DyahG_details{border-left:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1);flex-direction:column;flex:none;width:clamp(320px,38%,440px);min-width:0;max-width:calc(100% - 280px);min-height:0;display:flex;position:relative}._8DyahG_detailsResizeHandle{z-index:6;cursor:col-resize;touch-action:none;user-select:none;background:0 0;border:0;width:8px;padding:0;position:absolute;top:0;bottom:0;left:-4px}._8DyahG_detailsResizeHandle:focus-visible{outline:none}._8DyahG_detailsHeader{box-sizing:border-box;border-bottom:1px solid var(--dsw-alias-border-l2);flex:none;justify-content:space-between;align-items:center;height:42px;padding:0 8px 0 12px;display:flex}._8DyahG_detailsTitle{min-width:0;color:var(--dsw-alias-label-primary);align-items:center;gap:8px;display:flex}._8DyahG_requestDetailsDot{background:var(--dsw-alias-label-secondary);border-radius:50%;flex:none;width:5px;height:5px}._8DyahG_requestDetailsName{font:500 12px/16px var(--ds-font-family-code);flex:none}._8DyahG_detailsLocation{min-width:0;color:var(--dsw-alias-label-tertiary);font:11px/16px var(--ds-font-family-code);text-overflow:ellipsis;white-space:nowrap;overflow:hidden}._8DyahG_close{width:28px;height:28px;color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:0;border-radius:6px;flex:none;justify-content:center;align-items:center;padding:0;font-size:18px;line-height:18px;display:inline-flex}._8DyahG_close:hover{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-interactive-bg-hover)}._8DyahG_detailTabs{box-sizing:border-box;border-bottom:1px solid var(--dsw-alias-border-l2);overscroll-behavior-x:contain;scrollbar-width:none;white-space:nowrap;flex:none;gap:1px;width:100%;min-width:0;max-width:100%;height:34px;padding:0 8px;display:flex;overflow:auto hidden}._8DyahG_detailTabs::-webkit-scrollbar{display:none}._8DyahG_detailTab{color:var(--dsw-alias-label-tertiary);cursor:pointer;font:var(--dsw-font-xs-13);background:0 0;border:0;flex:none;padding:0 9px;position:relative}._8DyahG_detailTab:hover{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-interactive-bg-hover)}._8DyahG_detailTabActive{color:var(--dsw-alias-state-business-primary)}._8DyahG_detailTabActive:after{background:var(--dsw-alias-state-business-primary);content:\"\";border-radius:1px 1px 0 0;height:2px;position:absolute;bottom:0;left:9px;right:9px}._8DyahG_close:focus-visible,._8DyahG_detailTab:focus-visible{outline:1px solid var(--dsw-alias-state-business-primary);outline-offset:-1px}._8DyahG_detailBody{min-height:0;padding-bottom:var(--dsh-trajectory-bottom-clearance,0px);scrollbar-gutter:stable;flex:1;overflow:hidden auto}._8DyahG_detailBodySummary{box-sizing:border-box;padding-bottom:calc(12px + var(--dsh-trajectory-bottom-clearance,0px));flex-direction:column;display:flex;overflow:hidden}._8DyahG_detailBodySummary>._8DyahG_overview{overscroll-behavior:contain;flex:0 auto;min-height:0;overflow:auto}._8DyahG_detailBodySummary>._8DyahG_compactedSummary{flex:1;min-height:0;overflow:auto}._8DyahG_summaryScrollRegion{--dsh-scrollbar-thumb:transparent;--dsh-scrollbar-thumb-hover:transparent}._8DyahG_summaryScrollRegion:hover,._8DyahG_summaryScrollRegion:focus-within{--dsh-scrollbar-thumb:var(--dsw-alias-scrollbar-bg-l2);--dsh-scrollbar-thumb-hover:var(--dsw-alias-scrollbar-hover-l2)}._8DyahG_compactedSummary ._8DyahG_markdownPayload{padding-right:18px}._8DyahG_overview{font:var(--dsw-font-xs-13);margin:0;padding:8px 0}._8DyahG_overviewParentLinks{gap:14px;display:flex}._8DyahG_overviewHierarchyNavLink{all:unset;color:var(--dsw-alias-label-secondary);cursor:pointer;font:inherit;opacity:1;align-items:center;gap:1px;display:inline-flex}._8DyahG_overviewHierarchyNavLink:hover{color:var(--dsw-alias-label-primary)}._8DyahG_overviewHierarchyJumpIconTight{color:var(--dsw-alias-label-caption);flex:none}._8DyahG_overviewHierarchyNavLink:hover ._8DyahG_overviewHierarchyJumpIconTight,._8DyahG_overviewHierarchyNavLink:focus-visible ._8DyahG_overviewHierarchyJumpIconTight{color:var(--dsw-alias-label-primary)}._8DyahG_overviewHierarchyNavLink:focus-visible{color:var(--dsw-alias-label-primary);outline:1px solid var(--dsw-alias-state-business-primary);outline-offset:2px}._8DyahG_overview>div{grid-template-columns:94px minmax(0,1fr);align-items:center;min-height:22px;padding:0 14px;display:grid}._8DyahG_overview>._8DyahG_requestTokenDetail dt{padding-left:12px}._8DyahG_usagePanel{padding:4px 0 10px}._8DyahG_usageGroup+._8DyahG_usageGroup{margin-top:8px}._8DyahG_usageHeading{color:var(--dsw-alias-label-secondary);font:var(--dsw-font-xs-strong-13);user-select:none;margin:0;padding:4px 14px 1px}._8DyahG_usageGroup ._8DyahG_overview{padding:0}._8DyahG_overview dt{color:var(--dsw-alias-label-tertiary)}._8DyahG_overview dd{min-width:0;color:var(--dsw-alias-label-primary);text-overflow:ellipsis;white-space:nowrap;margin:0;overflow:hidden}._8DyahG_timestampToggle{all:unset;color:inherit;cursor:pointer;font:inherit;user-select:text}._8DyahG_timestampToggle:focus-visible{outline:1px solid var(--dsw-alias-state-business-primary);outline-offset:2px}._8DyahG_overviewSections{border-top:0;flex-direction:column;flex:1;min-height:min-content;display:flex;overflow:hidden}._8DyahG_overviewSection{flex-direction:column;flex:1 1 0;min-height:28px;max-height:max-content;display:flex;overflow:hidden}._8DyahG_overviewSection+._8DyahG_overviewSection{padding-top:8px}._8DyahG_overviewHeading{box-sizing:border-box;width:100%;height:28px;color:var(--dsw-alias-label-secondary);background:var(--dsw-alias-bg-layer-1);font:var(--dsw-font-xs-strong-13);user-select:none;flex:none;align-items:flex-end;margin:0;padding:0 0 3px 14px;display:flex}._8DyahG_overviewTitle{width:max-content;height:auto;color:inherit;cursor:pointer;font:inherit;user-select:none;background:0 0;border:0;flex:none;align-items:center;gap:3px;padding:0;line-height:1;display:inline-flex}._8DyahG_overviewTitleIcon{color:var(--dsw-alias-label-caption);flex:none}._8DyahG_overviewTitle:hover ._8DyahG_overviewTitleIcon{color:var(--dsw-alias-label-primary)}._8DyahG_overviewTitle:focus-visible{outline:1px solid var(--dsw-alias-state-business-primary);outline-offset:2px}._8DyahG_overviewTitle:focus-visible ._8DyahG_overviewTitleIcon{color:var(--dsw-alias-state-business-primary)}._8DyahG_overviewPreview{overscroll-behavior:contain;background:var(--dsw-alias-bg-layer-1);flex:1;min-height:0;overflow:auto}._8DyahG_overviewPreview>:last-child{margin-bottom:0;padding-bottom:0}._8DyahG_overviewPreview ._8DyahG_jsonPreview>:first-child,._8DyahG_overviewPreview ._8DyahG_schemaTree>:first-child{padding-bottom:0}._8DyahG_overviewPreview ._8DyahG_schemaTree{margin-bottom:0}._8DyahG_overviewPreview ._8DyahG_overview{padding:2px 0 0}._8DyahG_overviewPreview ._8DyahG_overview>div{min-height:22px}._8DyahG_markdownPreview,._8DyahG_markdownPayload{color:var(--dsw-alias-label-primary)}._8DyahG_markdownPreview>div,._8DyahG_markdownPayload>div{font:var(--dsw-font-xs-13);gap:8px}._8DyahG_markdownPreview>div h1,._8DyahG_markdownPayload>div h1{font:600 16px/22px var(--dsw-font-family)}._8DyahG_markdownPreview>div h2,._8DyahG_markdownPayload>div h2{font:600 15px/22px var(--dsw-font-family)}._8DyahG_markdownPreview>div :where(h3,h4,h5,h6),._8DyahG_markdownPayload>div :where(h3,h4,h5,h6){font:600 14px/20px var(--dsw-font-family)}._8DyahG_markdownPreview>div :not(pre)>code,._8DyahG_markdownPayload>div :not(pre)>code{font:12px/18px var(--ds-font-family-code)}._8DyahG_markdownPreview>div table,._8DyahG_markdownPayload>div table{font:var(--dsw-font-xs-13)}._8DyahG_markdownPreview>div>:first-child{margin-top:0}._8DyahG_markdownPreview>div>:last-child{margin-bottom:0}._8DyahG_markdownPreview>div :where(h1,h2,h3){margin:12px 0 6px}._8DyahG_markdownPreview>div :where(h4,h5,h6){margin:10px 0 5px}._8DyahG_markdownPreview>div :where(p,ul,ol){margin:8px 0}._8DyahG_markdownPreview>div li:not(:first-child){margin-top:3px}._8DyahG_markdownPreview>div blockquote{margin-top:8px}._8DyahG_markdownPreview>div hr{margin:14px 0}._8DyahG_markdownPreview>div>.md-code-block{margin:10px 0}._8DyahG_assistantContent ._8DyahG_markdownPayload,._8DyahG_assistantContent ._8DyahG_payload{min-height:0}._8DyahG_thinkingQuote{border-left:2px solid var(--dsw-alias-markdown-citation);color:var(--dsw-alias-label-secondary);background:0 0;margin:6px 14px 0 12px;padding-left:6px}._8DyahG_thinkingQuoteOnlyPreview{margin-bottom:8px}._8DyahG_detailBody>._8DyahG_assistantContentRendered ._8DyahG_thinkingQuote{margin-top:14px}._8DyahG_thinkingToggle{width:max-content;height:18px;color:var(--dsw-alias-label-tertiary);cursor:pointer;font:600 12px/18px var(--dsw-font-family);user-select:none;background:0 0;border:0;align-items:center;gap:2px;padding:0;display:flex}._8DyahG_thinkingChevron{transition:transform .12s var(--ds-ease-in-out);flex:none}._8DyahG_thinkingToggle[aria-expanded=true] ._8DyahG_thinkingChevron{transform:rotate(90deg)}._8DyahG_thinkingToggle:hover{color:var(--dsw-alias-label-secondary)}._8DyahG_thinkingToggle:focus-visible{outline:1px solid var(--dsw-alias-state-business-primary);outline-offset:2px}._8DyahG_thinkingQuote ._8DyahG_markdownPreview,._8DyahG_thinkingQuote ._8DyahG_markdownPayload,._8DyahG_thinkingQuote ._8DyahG_payload{min-height:0;color:var(--dsw-alias-label-secondary);background:0 0;padding:2px 0}._8DyahG_assistantOutput{background:var(--dsw-alias-bg-layer-1)}._8DyahG_markdownPreview{padding:6px 14px 8px}._8DyahG_markdownPayload{min-height:100%;padding:14px}._8DyahG_jsonPayload{min-height:100%}._8DyahG_jsonPreview{min-height:0}._8DyahG_schema{background:var(--dsw-alias-bg-layer-1);min-height:100%}._8DyahG_schemaPreview{min-height:0}._8DyahG_schemaIntro{padding:12px 14px 6px}._8DyahG_schemaPreview ._8DyahG_schemaIntro{padding-top:6px}._8DyahG_schemaName{color:var(--dsw-alias-label-primary);font:600 12px/18px var(--ds-font-family-code);margin:0}._8DyahG_schemaDescription{color:var(--dsw-alias-label-secondary);font:var(--dsw-font-xs-13);white-space:pre-wrap;margin:2px 0 0}._8DyahG_schemaParameters{min-width:0}._8DyahG_schemaParametersTitle{color:var(--dsw-alias-label-tertiary);font:600 11px/16px var(--dsw-font-family);user-select:none;margin:0;padding:4px 14px 2px}._8DyahG_schemaTree{margin:0 6px 8px 0}._8DyahG_payload{box-sizing:border-box;overflow-wrap:anywhere;min-height:100%;color:var(--dsw-alias-label-primary);background:var(--dsw-alias-markdown-code-block);font:12px/19px var(--ds-font-family-code);tab-size:2;white-space:pre-wrap;margin:0;padding:14px}._8DyahG_payloadPreview{min-height:0;padding:6px 14px 8px}._8DyahG_sourceBlocks{box-sizing:border-box;min-height:100%;padding:12px 14px}._8DyahG_sourceBlock+._8DyahG_sourceBlock{margin-top:14px}._8DyahG_sourceBlockHeader,._8DyahG_sourceBlockJumpTarget{user-select:none;align-items:center;gap:3px;width:max-content;margin-bottom:3px;display:flex}._8DyahG_sourceBlockJumpTarget{all:unset;cursor:pointer;user-select:none;align-items:center;gap:3px;width:max-content;margin-bottom:3px;display:flex}._8DyahG_sourceBlockLabel{color:var(--dsw-alias-label-tertiary);font:11px/16px var(--ds-font-family-code)}._8DyahG_sourceBlockJumpIcon{color:var(--dsw-alias-label-caption);flex:none}._8DyahG_sourceBlockJumpTarget:hover ._8DyahG_sourceBlockJumpIcon{color:var(--dsw-alias-label-primary)}._8DyahG_sourceBlockJumpTarget:focus-visible{outline:none}._8DyahG_sourceBlockJumpTarget:focus-visible ._8DyahG_sourceBlockJumpIcon{color:var(--dsw-alias-state-business-primary)}._8DyahG_sourceBlockContent{overflow-wrap:anywhere;color:var(--dsw-alias-label-primary);font:12px/19px var(--ds-font-family-code);tab-size:2;white-space:pre-wrap;background:0 0;margin:0}._8DyahG_panelImageLink{cursor:zoom-in;border-radius:4px;width:auto;max-width:100%;display:block;overflow:hidden}._8DyahG_panelImageLinkPreview{max-height:140px}._8DyahG_panelImage{border-radius:inherit;background:var(--dsw-alias-bg-base);object-fit:contain;width:auto;max-width:100%;height:auto;max-height:320px;margin:0;display:block}._8DyahG_panelImageLinkPreview ._8DyahG_panelImage{max-height:140px}._8DyahG_messageImages{flex-direction:column;gap:8px;margin:8px 14px 14px;display:flex}._8DyahG_messageImagesPreview{gap:6px;margin:6px 14px 8px}._8DyahG_assistantToolCalls{box-sizing:border-box;max-width:100%;color:var(--dsw-alias-label-secondary);font:11px/17px var(--ds-font-family-code);margin:2px 14px 12px;padding:0;list-style:none}._8DyahG_assistantToolCallsPreview{margin-top:2px;margin-bottom:8px}._8DyahG_assistantToolCalls li{min-width:0}._8DyahG_assistantToolCallButton{all:unset;box-sizing:border-box;cursor:pointer;border-radius:3px;align-items:center;width:calc(100% + 4px);min-width:0;height:19px;margin-left:-4px;padding:0 4px;display:flex}._8DyahG_assistantToolCallButton:hover{background:var(--dsw-alias-interactive-bg-hover)}._8DyahG_assistantToolCallButton:focus-visible{background:var(--dsw-alias-interactive-bg-hover);outline:1px solid var(--dsw-alias-state-business-primary);outline-offset:-1px}._8DyahG_assistantToolCallIcon{color:var(--dsw-alias-label-caption);flex:none;margin-right:5px}._8DyahG_assistantToolCallButton:hover ._8DyahG_assistantToolCallIcon,._8DyahG_assistantToolCallButton:focus-visible ._8DyahG_assistantToolCallIcon{color:var(--dsw-alias-label-secondary)}._8DyahG_assistantToolCallText{white-space:nowrap;flex:1;min-width:0;display:flex;overflow:hidden}._8DyahG_assistantToolCallName{color:var(--dsw-alias-label-secondary);flex:none;margin-right:5px;font-weight:500}._8DyahG_assistantToolCallArgs{min-width:0;color:var(--dsw-alias-label-tertiary);text-overflow:ellipsis;white-space:nowrap;overflow:hidden}._8DyahG_systemPrompt{box-sizing:border-box}._8DyahG_toolCatalog{min-height:100%;padding:8px 0 16px}._8DyahG_toolCatalogItem{border-bottom:1px solid var(--dsw-alias-border-l1)}._8DyahG_toolCatalogSummary{box-sizing:border-box;cursor:pointer;user-select:none;grid-template-columns:12px 12px max-content minmax(0,1fr);align-items:center;gap:5px;min-height:30px;padding:4px 12px;list-style:none;display:grid}._8DyahG_toolCatalogSummary::-webkit-details-marker{display:none}._8DyahG_toolCatalogSummary:hover{background:var(--dsw-alias-interactive-bg-hover)}._8DyahG_toolCatalogSummary:focus-visible{background:var(--dsw-alias-interactive-bg-hover);outline:1px solid var(--dsw-alias-state-business-primary);outline-offset:-1px}._8DyahG_toolCatalogChevron,._8DyahG_toolCatalogIcon{color:var(--dsw-alias-label-caption)}._8DyahG_toolCatalogChevron{transition:transform .1s var(--ds-ease-in-out)}._8DyahG_toolCatalogItem[open] ._8DyahG_toolCatalogChevron{transform:rotate(90deg)}._8DyahG_toolCatalogName{color:var(--dsw-alias-label-primary);font:500 12px/18px var(--ds-font-family-code)}._8DyahG_toolCatalogDescription{min-width:0;color:var(--dsw-alias-label-tertiary);font:var(--dsw-font-xs-13);text-overflow:ellipsis;white-space:nowrap;overflow:hidden}._8DyahG_toolCatalogDefinition{background:var(--dsw-alias-bg-base);padding:0 0 8px 29px}._8DyahG_toolCatalogFullDescription{color:var(--dsw-alias-label-secondary);font:var(--dsw-font-xs-13);white-space:pre-wrap;margin:0;padding:7px 14px 4px 0}._8DyahG_toolCatalogTree{margin-left:-14px;margin-right:6px}._8DyahG_resultBlocks{box-sizing:border-box;flex-direction:column;gap:10px;min-height:100%;padding:14px;display:flex}._8DyahG_resultBlocksPreview{gap:6px;min-height:0;padding:6px 14px 8px}._8DyahG_resultBlockText{overflow-wrap:anywhere;color:var(--dsw-alias-label-primary);font:12px/19px var(--ds-font-family-code);tab-size:2;white-space:pre-wrap;background:0 0;margin:0}._8DyahG_overviewPreview ._8DyahG_noPayload{padding:6px 14px 8px}._8DyahG_noPayload{color:var(--dsw-alias-label-tertiary);font:var(--dsw-font-xs-13);margin:0;padding:18px 14px}@media (width<=760px){._8DyahG_details{z-index:5;border-left-color:var(--dsw-alias-border-l3);width:min(92%,420px);max-width:92%;position:absolute;top:0;bottom:0;right:0;box-shadow:-12px 0 32px #00000024}}";
		const tagId$3 = "@xharness/dsh-client-ui-trajectory/TrajectoryTable.module.css";
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId$3) + "]") === null) {
			const tag = document.createElement("style");
			tag.dataset.plugin = "@xharness/dsh-client-ui-trajectory";
		...
		var TrajectoryTable_module_css_default = {
			"arrow": "_8DyahG_arrow",
			"assistantContent": "_8DyahG_assistantContent",
			"assistantContentRendered": "_8DyahG_assistantContentRendered",
			"assistantOutput": "_8DyahG_assistantOutput",
			"assistantToolCallArgs": "_8DyahG_assistantToolCallArgs",
			"assistantToolCallButton": "_8DyahG_assistantToolCallButton",
			"assistantToolCallIcon": "_8DyahG_assistantToolCallIcon",
			"assistantToolCallName": "_8DyahG_assistantToolCallName",
			"assistantToolCallText": "_8DyahG_assistantToolCallText",
			"assistantToolCalls": "_8DyahG_assistantToolCalls",
			"assistantToolCallsPreview": "_8DyahG_assistantToolCallsPreview",
			"assistantVioletBright": "_8DyahG_assistantVioletBright",
			"close": "_8DyahG_close",
			"collapsedTurnContent": "_8DyahG_collapsedTurnContent",
			"collapsedTurnEllipsis": "_8DyahG_collapsedTurnEllipsis",
			"collapsedTurnText": "_8DyahG_collapsedTurnText",
			"compacted": "_8DyahG_compacted",
			"compactedSummary": "_8DyahG_compactedSummary",
			"content": "_8DyahG_content",
			"contentColumn": "_8DyahG_contentColumn",
			"contentText": "_8DyahG_contentText",
			"contextGreen": "_8DyahG_contextGreen",
			"detailBody": "_8DyahG_detailBody",
			"detailBodySummary": "_8DyahG_detailBodySummary",
			"detailTab": "_8DyahG_detailTab",
			"detailTabActive": "_8DyahG_detailTabActive",
			"detailTabs": "_8DyahG_detailTabs",
			"details": "_8DyahG_details",
			"detailsHeader": "_8DyahG_detailsHeader",
			"detailsLocation": "_8DyahG_detailsLocation",
			"detailsResizeHandle": "_8DyahG_detailsResizeHandle",
			"detailsTitle": "_8DyahG_detailsTitle",
			"error": "_8DyahG_error",
			"errorPayload": "_8DyahG_errorPayload",
			"event": "_8DyahG_event",
			"eventColumn": "_8DyahG_eventColumn",
			"eventHeader": "_8DyahG_eventHeader",
			"eventInner": "_8DyahG_eventInner",
			"history-loading-spin": "_8DyahG_history-loading-spin",
			"historyLoadButton": "_8DyahG_historyLoadButton",
			"historyLoadRow": "_8DyahG_historyLoadRow",
			"historyLoading": "_8DyahG_historyLoading",
			"historyLoadingBar": "_8DyahG_historyLoadingBar",
			"historyLoadingSpinner": "_8DyahG_historyLoadingSpinner",
			"inlineResult": "_8DyahG_inlineResult",
			"inlineResultText": "_8DyahG_inlineResultText",
			"jsonPayload": "_8DyahG_jsonPayload",
			"jsonPreview": "_8DyahG_jsonPreview",
			"kindSlot": "_8DyahG_kindSlot",
			"kindTag": "_8DyahG_kindTag",
			"kindTagIcon": "_8DyahG_kindTagIcon",
			"kindTagLabel": "_8DyahG_kindTagLabel",
			"markdownPayload": "_8DyahG_markdownPayload",
			"markdownPreview": "_8DyahG_markdownPreview",
			"message": "_8DyahG_message",
			"messageImages": "_8DyahG_messageImages",
			"messageImagesPreview": "_8DyahG_messageImagesPreview",
			"noOutputText": "_8DyahG_noOutputText",
			"noPayload": "_8DyahG_noPayload",
			"overview": "_8DyahG_overview",
			"overviewHeading": "_8DyahG_overviewHeading",
			"overviewHierarchyJumpIconTight": "_8DyahG_overviewHierarchyJumpIconTight",
			"overviewHierarchyNavLink": "_8DyahG_overviewHierarchyNavLink",
			"overviewParentLinks": "_8DyahG_overviewParentLinks",
			"overviewPreview": "_8DyahG_overviewPreview",
			"overviewSection": "_8DyahG_overviewSection",
			"overviewSections": "_8DyahG_overviewSections",
			"overviewTitle": "_8DyahG_overviewTitle",
			"overviewTitleIcon": "_8DyahG_overviewTitleIcon",
			"panelImage": "_8DyahG_panelImage",
			"panelImageLink": "_8DyahG_panelImageLink",
			"panelImageLinkPreview": "_8DyahG_panelImageLinkPreview",
			"payload": "_8DyahG_payload",
			"payloadPreview": "_8DyahG_payloadPreview",
			"promptDiff": "_8DyahG_promptDiff",
			"promptDiffLineadded": "_8DyahG_promptDiffLineadded",
			"promptDiffLinecontext": "_8DyahG_promptDiffLinecontext",
			"promptDiffLinemeta": "_8DyahG_promptDiffLinemeta",
			"promptDiffLineremoved": "_8DyahG_promptDiffLineremoved",
			"promptDiffSection": "_8DyahG_promptDiffSection",
			"promptDiffSections": "_8DyahG_promptDiffSections",
			"promptDiffTitle": "_8DyahG_promptDiffTitle",
			"requestBoundaryControl": "_8DyahG_requestBoundaryControl",
			"requestBoundaryControlActive": "_8DyahG_requestBoundaryControlActive",
			"requestDetailsDot": "_8DyahG_requestDetailsDot",
			"requestDetailsName": "_8DyahG_requestDetailsName",
			"requestTokenDetail": "_8DyahG_requestTokenDetail",
			"resultBlockText": "_8DyahG_resultBlockText",
			"resultBlocks": "_8DyahG_resultBlocks",
			"resultBlocksPreview": "_8DyahG_resultBlocksPreview",
			"resultPreview": "_8DyahG_resultPreview",
			"resultRequest": "_8DyahG_resultRequest",
			"schema": "_8DyahG_schema",
			"schemaDescription": "_8DyahG_schemaDescription",
			"schemaIntro": "_8DyahG_schemaIntro",
			"schemaName": "_8DyahG_schemaName",
			"schemaParameters": "_8DyahG_schemaParameters",
			"schemaParametersTitle": "_8DyahG_schemaParametersTitle",
			"schemaPreview": "_8DyahG_schemaPreview",
			"schemaTree": "_8DyahG_schemaTree",
			"selectionRail": "_8DyahG_selectionRail",
			"sourceBlock": "_8DyahG_sourceBlock",
			"sourceBlockContent": "_8DyahG_sourceBlockContent",
			"sourceBlockHeader": "_8DyahG_sourceBlockHeader",
			"sourceBlockJumpIcon": "_8DyahG_sourceBlockJumpIcon",
			"sourceBlockJumpTarget": "_8DyahG_sourceBlockJumpTarget",
			"sourceBlockLabel": "_8DyahG_sourceBlockLabel",
			"sourceBlocks": "_8DyahG_sourceBlocks",
			"split": "_8DyahG_split",
			"subtoolAmber": "_8DyahG_subtoolAmber",
			"summaryScrollRegion": "_8DyahG_summaryScrollRegion",
			"systemNeutral": "_8DyahG_systemNeutral",
			"systemPrompt": "_8DyahG_systemPrompt",
			"table": "_8DyahG_table",
			"tablePane": "_8DyahG_tablePane",
			"thinkingChevron": "_8DyahG_thinkingChevron",
			"thinkingQuote": "_8DyahG_thinkingQuote",
			"thinkingQuoteOnlyPreview": "_8DyahG_thinkingQuoteOnlyPreview",
			"thinkingToggle": "_8DyahG_thinkingToggle",
			"timestampToggle": "_8DyahG_timestampToggle",
			"toolAmber": "_8DyahG_toolAmber",
			"toolCallNameTypeface": "_8DyahG_toolCallNameTypeface",
			"toolCallOnly": "_8DyahG_toolCallOnly",
			"toolCallPayload": "_8DyahG_toolCallPayload",
			"toolCatalog": "_8DyahG_toolCatalog",
			"toolCatalogChevron": "_8DyahG_toolCatalogChevron",
			"toolCatalogDefinition": "_8DyahG_toolCatalogDefinition",
			"toolCatalogDescription": "_8DyahG_toolCatalogDescription",
			"toolCatalogFullDescription": "_8DyahG_toolCatalogFullDescription",
			"toolCatalogIcon": "_8DyahG_toolCatalogIcon",
			"toolCatalogItem": "_8DyahG_toolCatalogItem",
			"toolCatalogName": "_8DyahG_toolCatalogName",
			"toolCatalogSummary": "_8DyahG_toolCatalogSummary",
			"toolCatalogTree": "_8DyahG_toolCatalogTree",
			"trajectory-table": "_8DyahG_trajectory-table",
			"turnLabel": "_8DyahG_turnLabel",
			"turnLabelActive": "_8DyahG_turnLabelActive",
			"turnLabelCompact": "_8DyahG_turnLabelCompact",
			"turnLabelFull": "_8DyahG_turnLabelFull",
			"turnRail": "_8DyahG_turnRail",
			"usageGroup": "_8DyahG_usageGroup",
			"usageHeading": "_8DyahG_usageHeading",
			"usagePanel": "_8DyahG_usagePanel",
			"user": "_8DyahG_user",
			"virtualSpacer": "_8DyahG_virtualSpacer",
			"visuallyHidden": "_8DyahG_visuallyHidden"
		};
		//#endregion
		...
		const KIND_ICON = {
			system: (0, react_jsx_runtime.jsx)(_xharness_dsh_client_ui_primitives.IconSettingsOutline16, { size: 13 }),
			user: (0, react_jsx_runtime.jsx)(_xharness_dsh_client_ui_primitives.IconUserOutline16, { size: 13 }),
			context: (0, react_jsx_runtime.jsx)(InformationIcon, {}),
			compacted: (0, react_jsx_runtime.jsx)(CompactedIcon, {}),
			message: (0, react_jsx_runtime.jsx)(_xharness_dsh_client_ui_primitives.IconSparkle16, { size: 13 }),
		...
		function RequestOptions({ options, preview = false }) {
			...
			return (0, react_jsx_runtime.jsx)(_xharness_dsh_client_ui_primitives.JsonTree, {
				data: options,
		...
		function MarkdownFragment({ text, rendered, preview }) {
			if (rendered) return (0, react_jsx_runtime.jsx)("div", {
				...
				children: (0, react_jsx_runtime.jsx)(_xharness_dsh_client_ui_primitives.MarkdownText, { text })
			});
			...
			return (0, react_jsx_runtime.jsx)("div", {
				...
				children: blocks.map((block, index) => (0, react_jsx_runtime.jsxs)("section", {
					...
					children: [block.callId !== void 0 ? (0, react_jsx_runtime.jsxs)("button", {
						...
						}), (0, react_jsx_runtime.jsx)(_xharness_dsh_client_ui_primitives.IconChevronRightOutline14, {
				...
				children: tools.map((tool, index) => (0, react_jsx_runtime.jsxs)("details", {
					...
					children: [(0, react_jsx_runtime.jsxs)("summary", {
						...
						children: [tool.description !== "" && (0, react_jsx_runtime.jsx)("p", {
							...
							children: tool.description
						}), (0, react_jsx_runtime.jsx)(_xharness_dsh_client_ui_primitives.JsonTree, {
					...
					children: [
						(0, react_jsx_runtime.jsxs)("div", {
							...
							children: [(0, react_jsx_runtime.jsxs)("button", {
								...
								children: ["Thinking", (0, react_jsx_runtime.jsx)(_xharness_dsh_client_ui_primitives.IconChevronRightOutline14, {
		...
		function RecordPayload({ record, direction, preview = false }) {
			...
			if (direction === "output" && record.cell.outputBlocks?.length === 1 && record.cell.outputBlocks[0]?.type === "text" && json !== void 0) return (0, react_jsx_runtime.jsx)(_xharness_dsh_client_ui_primitives.JsonTree, {
				data: json,
				label: "Result JSON",
			...
			if (direction === "input" && (record.cell.kind === "user" || record.cell.kind === "context") || direction === "output" && record.cell.kind === "message") return (0, react_jsx_runtime.jsx)("div", {
				...
				children: (0, react_jsx_runtime.jsx)(_xharness_dsh_client_ui_primitives.MarkdownText, { text: value })
			});
			if (json !== void 0) return (0, react_jsx_runtime.jsx)(_xharness_dsh_client_ui_primitives.JsonTree, {
				data: json,
			...
			return (0, react_jsx_runtime.jsxs)("section", {
				...
				children: [(0, react_jsx_runtime.jsx)("h3", {
					...
					children: (0, react_jsx_runtime.jsxs)("button", {
						...
						onClick: onOpen,
						children: [(0, react_jsx_runtime.jsx)("span", { children: label }), (0, react_jsx_runtime.jsx)(_xharness_dsh_client_ui_primitives.IconChevronRightOutline14, {
		...
		*/
						...
						(0, react_jsx_runtime.jsxs)("div", {
							...
							children: [
									...
									children: [
										...
										selectedRequestResult !== void 0 && (0, react_jsx_runtime.jsxs)("div", { children: [(0, react_jsx_runtime.jsx)("dt", { children: "Result" }), (0, react_jsx_runtime.jsx)("dd", {
											...
											children: (0, react_jsx_runtime.jsxs)("button", {
												...
												children: [(0, react_jsx_runtime.jsx)("span", { children: selectedRequestInfo?.purpose === "compaction" ? "Compacted" : "Assistant Message" }), (0, react_jsx_runtime.jsx)(_xharness_dsh_client_ui_primitives.IconChevronRightOutline14, {
								...
								promptSelected && activeTab === "system-prompt" && (selectedPrompt.system === "" ? (0, react_jsx_runtime.jsx)("p", {
									...
									children: (0, react_jsx_runtime.jsx)(_xharness_dsh_client_ui_primitives.MarkdownText, { text: selectedPrompt.system })
								})),
									...
									children: [
										selected.cell.messageSource !== void 0 && (0, react_jsx_runtime.jsxs)("div", { children: [(0, react_jsx_runtime.jsx)("dt", { children: "Source" }), (0, react_jsx_runtime.jsx)("dd", {
											...
											children: (0, react_jsx_runtime.jsxs)("button", {
												...
												children: [(0, react_jsx_runtime.jsx)("span", { children: messageSourceLabel(selected.cell.messageSource) }), (0, react_jsx_runtime.jsx)(_xharness_dsh_client_ui_primitives.IconChevronRightOutline14, {
													className: TrajectoryTable_module_css_default.overviewHierarchyJumpIconTight,
													size: 11
												})]
											})
										...
										hasSelectedHierarchy && (0, react_jsx_runtime.jsxs)("div", { children: [(0, react_jsx_runtime.jsx)("dt", { children: selectedAssistantRequestTarget !== void 0 ? "Source" : "Hierarchy" }), (0, react_jsx_runtime.jsxs)("dd", {
											...
											children: [
												selectedAssistantRequestTarget !== void 0 && (0, react_jsx_runtime.jsxs)("button", {
													...
													children: [(0, react_jsx_runtime.jsxs)("span", { children: ["Request #", selectedAssistantRequest ?? "—"] }), (0, react_jsx_runtime.jsx)(_xharness_dsh_client_ui_primitives.IconChevronRightOutline14, {
														className: TrajectoryTable_module_css_default.overviewHierarchyJumpIconTight,
														size: 11
													})]
												}),
												selectedParentMessage !== void 0 && (0, react_jsx_runtime.jsxs)("button", {
													...
													children: [(0, react_jsx_runtime.jsx)("span", { children: "Assistant Message" }), (0, react_jsx_runtime.jsx)(_xharness_dsh_client_ui_primitives.IconChevronRightOutline14, {
														className: TrajectoryTable_module_css_default.overviewHierarchyJumpIconTight,
														size: 11
													})]
												}),
												selectedParentTool !== void 0 && (0, react_jsx_runtime.jsxs)("button", {
													...
													children: [(0, react_jsx_runtime.jsx)("span", { children: "Tool Call" }), (0, react_jsx_runtime.jsx)(_xharness_dsh_client_ui_primitives.IconChevronRightOutline14, {
														className: TrajectoryTable_module_css_default.overviewHierarchyJumpIconTight,
														size: 11
													})]
												})
		...
		//#endregion
		//#region \0dsh-css:deepseek-harness/packages/client/ui-trajectory/src/client/TrajectoryToolbar.module.css.mjs
		const css$2 = ".D65Vfa_root{z-index:4;box-sizing:border-box;width:100%;height:var(--dsh-trajectory-toolbar-height);border-bottom:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1);position:sticky;top:0}.D65Vfa_inner{box-sizing:border-box;align-items:center;gap:8px;width:100%;height:100%;padding:0 6px;display:flex}.D65Vfa_actions{flex:none;align-items:center;gap:2px;display:flex}.D65Vfa_toggle{height:20px;color:var(--dsw-alias-label-tertiary);cursor:pointer;font:var(--dsw-font-xxs-12);background:0 0;border:0;border-radius:3px;flex:none;align-items:center;gap:4px;padding:0 7px;display:inline-flex}.D65Vfa_toggle:hover,.D65Vfa_toggle[aria-pressed=true]{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-interactive-bg-hover)}.D65Vfa_toggle:focus-visible{outline:1px solid var(--dsw-alias-state-business-primary);outline-offset:1px}.D65Vfa_toggleIcon{stroke:currentColor;stroke-width:1.25px;stroke-linecap:round;stroke-linejoin:round;flex:none;width:12px;height:12px}.D65Vfa_control{box-sizing:border-box;width:88px;height:20px;color:var(--dsw-alias-label-tertiary);cursor:pointer;font:var(--dsw-font-xxs-12);background:0 0;border:0;border-radius:0;flex:none;justify-content:center;align-items:center;gap:4px;padding:0 5px;display:inline-flex}.D65Vfa_control[hidden]{display:none}.D65Vfa_control:hover:not(:disabled),.D65Vfa_control[aria-checked=true],.D65Vfa_control[aria-pressed=true]{color:var(--dsw-alias-label-primary)}.D65Vfa_control:focus-visible{outline:1px solid var(--dsw-alias-state-business-primary);outline-offset:1px}.D65Vfa_control:disabled{color:var(--dsw-alias-label-dimmed);cursor:not-allowed}.D65Vfa_controlTrack{background:var(--dsw-alias-border-l2);width:20px;height:10px;transition:background-color .12s var(--ds-ease-in-out);border-radius:5px;flex:none;display:inline-block;position:relative}.D65Vfa_controlThumb{background:var(--dsw-alias-bg-layer-1);width:6px;height:6px;transition:transform .12s var(--ds-ease-in-out);border-radius:50%;position:absolute;top:2px;left:2px}.D65Vfa_controlTrack[data-on=true]{background:var(--dsw-alias-state-business-primary)}.D65Vfa_controlTrack[data-on=true] .D65Vfa_controlThumb{transform:translate(10px)}.D65Vfa_action{height:20px;color:var(--dsw-alias-label-tertiary);cursor:pointer;font:var(--dsw-font-xxs-12);background:0 0;border:0;border-radius:3px;flex:none;align-items:center;gap:4px;padding:0 5px;display:inline-flex}.D65Vfa_action:hover{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-interactive-bg-hover)}.D65Vfa_action:focus-visible{outline:1px solid var(--dsw-alias-state-business-primary);outline-offset:1px}.D65Vfa_actionIcon{color:var(--dsw-alias-label-tertiary);font:14px/14px var(--ds-font-family-code)}.D65Vfa_search{border:1px solid var(--dsw-alias-border-l2);min-width:84px;height:22px;color:var(--dsw-alias-label-caption);background:var(--dsw-alias-bg-layer-2);border-radius:4px;flex:0 164px;align-items:center;gap:4px;margin-left:auto;padding:0 6px;display:flex}.D65Vfa_search:hover{border-color:var(--dsw-alias-label-caption)}.D65Vfa_search:focus-within{border-color:var(--dsw-alias-state-business-primary);background:var(--dsw-alias-bg-layer-1)}.D65Vfa_searchIcon{flex:none}.D65Vfa_searchInput{width:100%;min-width:0;color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxs-12);background:0 0;border:0;outline:0;padding:0}.D65Vfa_searchInput::placeholder{color:var(--dsw-alias-label-caption)}.D65Vfa_searchInput::-webkit-search-cancel-button{cursor:pointer;width:12px;height:12px}";
		const tagId$2 = "@xharness/dsh-client-ui-trajectory/TrajectoryToolbar.module.css";
			...
			return (0, react_jsx_runtime.jsx)("div", {
				...
				children: (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [(0, react_jsx_runtime.jsxs)("div", {
						...
						children: [(0, react_jsx_runtime.jsx)(_xharness_dsh_client_ui_primitives.IconSearchOutline16, {
							size: 11,
		...
		*/
		...
		//#endregion
		//#region \0dsh-css:deepseek-harness/packages/client/ui-trajectory/src/client/TrajectoryTimeline.module.css.mjs
		const css$1 = ".YDLR1G_root{z-index:1;isolation:isolate;border-bottom:1px solid var(--dsw-alias-border-l2);user-select:none;flex:none;position:relative}.YDLR1G_root [role=tooltip]{font:var(--dsw-font-xxxs-11)}.YDLR1G_plot{background:var(--dsw-alias-bg-layer-2);grid-template-columns:44px minmax(0,1fr);height:50px;display:grid;overflow:hidden}.YDLR1G_labels{border-right:1px solid var(--dsw-alias-border-l1);color:var(--dsw-alias-label-caption);font:var(--dsw-font-xs-13);font-size:10px;line-height:1;position:relative}.YDLR1G_labels span{text-align:right;justify-content:flex-end;align-items:center;height:8px;display:flex;position:absolute;right:3px}.YDLR1G_labels span:first-child{top:7px}.YDLR1G_labels span:nth-child(2){top:21px}.YDLR1G_labels span:nth-child(3){top:35px}.YDLR1G_track{cursor:crosshair;touch-action:none;position:relative;overflow:hidden}.YDLR1G_track[data-panning=true]{cursor:grabbing}.YDLR1G_earlierHistory{z-index:5;appearance:none;box-sizing:border-box;background:linear-gradient(to right, var(--dsw-alias-bg-layer-2) 0, var(--dsw-alias-bg-layer-2) 38%, transparent 100%);width:28px;color:var(--dsw-alias-label-secondary);font:var(--dsw-font-xs-13);opacity:.72;cursor:pointer;border:0;outline:none;justify-content:flex-start;align-items:center;padding-left:3px;line-height:1;display:flex;position:absolute;top:0;bottom:0;left:0}.YDLR1G_earlierHistory:hover{opacity:1}.YDLR1G_earlierHistory[aria-disabled=true]{cursor:default}.YDLR1G_earlierHistory:focus-visible{box-shadow:inset 0 0 0 1px var(--dsw-alias-border-l2)}.YDLR1G_empty{color:var(--dsw-alias-label-caption);font:var(--dsw-font-xs-13);position:absolute;top:50%;left:50%;transform:translate(-50%,-50%)}.YDLR1G_track:focus-visible{outline:1px solid var(--dsw-alias-state-business-primary);outline-offset:-1px}.YDLR1G_lanes{z-index:2;top:7px;bottom:7px;left:var(--trajectory-domain-left);width:var(--trajectory-domain-width);position:absolute}.YDLR1G_turnBoundaries{z-index:3;top:0;bottom:0;left:var(--trajectory-domain-left);width:var(--trajectory-domain-width);pointer-events:none;position:absolute}@media (prefers-reduced-motion:no-preference){.YDLR1G_lanes[data-animate-viewport=true],.YDLR1G_turnBoundaries[data-animate-viewport=true]{transition:left .18s ease-out}}.YDLR1G_turnBoundary{top:0;bottom:0;left:var(--trajectory-turn-left);background:var(--dsw-alias-border-l2);width:1px;position:absolute}.YDLR1G_span{top:calc(var(--trajectory-span-lane) * 14px);left:calc(var(--trajectory-span-left) + var(--trajectory-span-gap));width:max(2px, calc(var(--trajectory-span-width) - var(--trajectory-span-gap) - var(--trajectory-span-gap)));background:var(--dsw-alias-label-secondary);opacity:.78;border-radius:1px;min-width:2px;height:8px;position:absolute}.YDLR1G_span[data-timeline-span=user]{background:var(--dsw-alias-state-business-primary)}.YDLR1G_span[data-timeline-span=context]{background:color-mix(in srgb, var(--dsw-alias-state-success-primary) 68%, var(--dsw-alias-label-secondary))}.YDLR1G_span[data-timeline-span=message]{--trajectory-assistant-decoding-color:color-mix(in srgb, var(--dsw-alias-brand-primary-new-colorprimary-new-color) 60%, var(--dsw-alias-state-error-secondary));--trajectory-assistant-ttft-color:color-mix(in srgb, var(--trajectory-assistant-decoding-color) 54%, var(--dsw-alias-bg-layer-2));background:var(--trajectory-assistant-decoding-color);opacity:1}.YDLR1G_span[data-timeline-span=message][data-assistant-timing=true]{background:linear-gradient(to right, var(--trajectory-assistant-ttft-color) 0, var(--trajectory-assistant-ttft-color) var(--trajectory-assistant-ttft), var(--trajectory-assistant-decoding-color) var(--trajectory-assistant-ttft), var(--trajectory-assistant-decoding-color) 100%)}.YDLR1G_span[data-timeline-span=tool],.YDLR1G_span[data-timeline-span=subtool]{background:var(--dsw-alias-state-warn-label);opacity:1}.YDLR1G_span[data-error=true]{background:var(--dsw-alias-state-error-primary)}.YDLR1G_span[data-equal-duration=true]{width:8px;min-width:8px}.YDLR1G_span[data-selected=false]{opacity:.2}.YDLR1G_span[data-hovered=true]:not([data-current=true]){z-index:1;opacity:1;box-shadow:0 0 0 1px var(--dsw-alias-bg-layer-2), 0 0 0 2px color-mix(in srgb, var(--dsw-alias-state-business-primary) 80%, transparent)}.YDLR1G_span[data-current=true]{z-index:1;opacity:1;box-shadow:0 0 0 1px var(--dsw-alias-bg-layer-2), 0 0 0 2px var(--dsw-alias-state-business-primary)}.YDLR1G_span[data-search-match=false]{opacity:.14}.YDLR1G_selection{z-index:1;top:0;bottom:0;left:var(--trajectory-selection-left);width:var(--trajectory-selection-width);background:color-mix(in srgb, var(--dsw-alias-state-business-primary) 12%, transparent);min-width:1px;box-shadow:-100vw 0 0 100vw color-mix(in srgb, var(--dsw-alias-bg-layer-1) 58%, transparent), 100vw 0 0 100vw color-mix(in srgb, var(--dsw-alias-bg-layer-1) 58%, transparent);pointer-events:none;position:absolute}.YDLR1G_selectionEdges{z-index:4;top:0;bottom:0;left:var(--trajectory-selection-left);width:var(--trajectory-selection-width);pointer-events:none;min-width:1px;position:absolute}.YDLR1G_hoverLine{z-index:4;top:0;bottom:0;left:clamp(0px, calc(var(--trajectory-hover-left) - 1px), calc(100% - 2px));background:var(--dsw-alias-state-business-primary);pointer-events:none;width:2px;position:absolute}.YDLR1G_selectionEdges:before,.YDLR1G_selectionEdges:after{background:var(--dsw-alias-state-business-primary);content:\"\";width:3px;position:absolute;top:0;bottom:0}.YDLR1G_selectionEdges:before{left:0}.YDLR1G_selectionEdges:after{right:0}.YDLR1G_selectionEdges[data-dragging=true]:before,.YDLR1G_selectionEdges[data-dragging=true]:after{width:2px}.YDLR1G_selection[data-dragging=true]{background:color-mix(in srgb, var(--dsw-alias-state-business-primary) 18%, transparent)}";
		const tagId$1 = "@xharness/dsh-client-ui-trajectory/TrajectoryTimeline.module.css";
		...
		function EarlierHistoryBoundary({ loading, onHover, onLoad }) {
			return (0, react_jsx_runtime.jsx)(_xharness_dsh_client_ui_primitives.Tooltip, {
		...
		*/
		...
		//#endregion
		//#region \0dsh-css:deepseek-harness/packages/client/ui-trajectory/src/client/views.module.css.mjs
		const css = ".mCzLya_root{--dsh-trajectory-toolbar-height:32px;box-sizing:border-box;width:100%;height:100%;min-height:0;color:var(--dsw-alias-label-primary);background:var(--dsw-alias-bg-layer-1);flex-direction:column;display:flex;overflow:hidden}.mCzLya_ledger{z-index:0;isolation:isolate;--dsh-trajectory-bottom-clearance:calc(var(--dsh-composer-height,152px) + 16px);flex:1;min-width:0;min-height:0;display:flex;position:relative;overflow:hidden}";
		const tagId = "@xharness/dsh-client-ui-trajectory/views.module.css";
